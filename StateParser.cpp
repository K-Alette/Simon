...
void StateParser::parseTextures(TiXmlElement* pStateRoot, std::vector<std::string>* pTextureIDs)
{
	for (TiXmlElement* e = pStateRoot->FirstChildElement(); e != NULL; e = e->NextSiblingElement())
	{
		...
		TheTextureManager::Instance()->load(filenameAttribute, idAttribute, Simon::Instance()->getRenderer());
	}
}

void StateParser::parseObjects(TiXmlElement* pStateRoot, std::vector<GameObjects*>* pObjects)
{
	for (TiXmlElement* e = pStateRoot->FirstChildElement(); e != NULL; e = e->NextSiblingElement())
	{
		...
		GameObjects* pGameObject = TheGameObjectFactory::Instance()->create(e->Attribute("type"));
